                ...
                if (c.moveToFirst()) {
                    do {
                        ...
                        if (cal_in_food.moveToFirst()) {
                            Cursor food_num = db.rawQuery("SELECT * FROM record WHERE date = '" + eat_page_activity.selectdate + "' AND category = '" + eat_new_second.category + "' AND foodID = " + c.getInt(3), null);
